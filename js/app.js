...
$(document).ready(function () {
	/*Таблица на 3 странице загораются*/
	$('.block4__item').click(function () {
		$('.block4__item').removeAttr('id');
		$(this).attr("id", "active");
		$(this).addClass("visited");
	});
	/*Меню мобильное в шапке, открыть*/
	$('#menu-toggle').click(function () {
		$('.mob-menu').toggle(100);
		$(this).toggleClass('open');
	});
	/*1 страница таблица мобильная, открыть*/
	$('.table-mob__open').click(function () {
		if ($(this).parent().children('.table-mob__books').is(':visible')) {
			$(this).parent().children('.table-mob__books').hide();
		} else {
			$('.table-mob__books').hide();
			$(this).parent().children('.table-mob__books').toggle();
		}
	});
	/*Поиск, подсказка*/
	$('.search__input').mouseenter(function () {
		$('.search__list').toggle();
	});
	$('.search__input').mouseleave(function () {
		$('.search__list').toggle();
	});
});